...
// Update: This question has been changed after the contest.
// It added the special restriction 0 < nums[i] < 10000.
...
// we will start from position 0 in nums[] to look up the elements that
// are not used yet and find another cur_sum = sum/k.

// An corner case is when sum = 0, my method is to use cur_num to
// record the number of elements in the current subset.
// Only if cur_sum = sum/k && cur_num >0,
